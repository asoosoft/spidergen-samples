...
afc.extendsClass = function(childClass, parentClass)
{
	...
	//childClass 를 셋팅한다. 호출하는 시점에서는 자기 자신이다.
	//afc.setScriptMap();
...
afc.loadScript = function(url)
{
	...
	if(!afc.scriptMap[url])
	{
		...
		$('<script defer src="' + url + '"></script>').appendTo('head');
	}
...
console.log(meta);
	...
	$('<meta http-equiv="Content-Security-Policy" content="connect-src *; default-src * gap://ready file:; img-src * data:; style-src * \'unsafe-inline\'; script-src * \'unsafe-inline\' \'unsafe-eval\'">').prependTo('head');
...
afc.deviceCheck = function()
{
	...
	if(afc.isMobile)
	{
		...
		//모바일일때만 키보드매니저를 동적으로 추가해준다.
		afc.loadScript('Framework/afc/library/KeyboardManager.js');
	}
	...
	if(window.PROJECT_OPTION)
	{
		if(PROJECT_OPTION.build.bridgeName=='cordova')
		{
			...
			if(afc.isSimulator) afc.loadScript('Bridge/windows/cordova.js');
			else if(afc.isIos) afc.loadScript('Bridge/ios/cordova.js');
